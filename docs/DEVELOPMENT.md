...
├── internal/
│   ├── product/                  # Product Application、Node Catalog/Config Schema、Workflow identity、LLM 设置、Run 日志与 Crash bundle
...
│   ├── redaction/                # 共享 Secret/Header 脱敏 seam（日志、错误与诊断包）
...
│   │   ├── builtins/             # Mock/Human Executor、四个真实 Go 质量检查与内嵌定义/Script Bundle
...
product ──> product/workflow / artifact / chat / runtimepath / secret / redaction <── history
chat ──> 标准库
redaction ──> 标准库
