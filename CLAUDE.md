...
**14 后 Product Workflow 01–08 已完成**：Wails macOS 产品壳、Browser Mock 与 Desktop Adapter 共享 WorkflowClient / Product Application seam；用户可以创建 SQLite Product Workflow，编辑唯一 Draft，从 Catalog 创作 `human-chat` / `llm-chat`、Input Binding / Control Dependency，并管理 SQLite `Provider -> Models`。Draft autosave 使用 expected `lock_version` CAS，Preview 在非法 Draft 下仍返回可识别图与聚合 Diagnostics。P9 `StartRun` 校验 UI token、物化默认 Gum Model UUID、创建或复用 immutable Revision、固定无 Secret 的 Run Snapshot，并由 deterministic fake executor 持久化 Run、Node Run 与 filesystem-backed Conversation Artifact；Desktop UI 可查看本次结果。P9 后 Revision reuse 与分层 Run history 已落地：相同规范化语义哈希重复运行复用同一 immutable Revision，每次创建新 Run；展示文案、Presentation Hint 与 UI view preference 不进入哈希；UI 可按 Workflow → Revision 列表 → 每个 Revision 的 Runs → 每次 Run 的 Node Run/Artifact 摘要逐层浏览，Conversation 消息从 filesystem Artifact Store 还原，应用重启后历史仍可查询。分层历史为只读浏览，Interrupted 标记与 Resume 仍属后续。独立 product schema 不读取或复用 workflow/v1 YAML identity。真实 LLM、人工输入尚未实现；下一票是 `.scratch/product-workflow/issues/09-macos-keychain-secret-adapter.md`。
