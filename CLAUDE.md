...
**14 后 Product Workflow 01–15 已完成**：Wails macOS 产品壳、Browser Mock 与 Desktop Adapter 共享 WorkflowClient / Product Application seam；用户可以创建 SQLite Product Workflow，编辑唯一 Draft，从 Catalog 创作 `human-chat` / `llm-chat`、Input Binding / Control Dependency，并管理 SQLite `Provider -> Models`。Draft autosave 使用 expected `lock_version` CAS，Preview 在非法 Draft 下仍返回可识别图与聚合 Diagnostics。`StartRun` 校验 UI token、物化默认 Gum Model UUID、创建或复用 immutable Revision、固定不含 Secret 明文的 Run Snapshot；P10 起单轮 `human-chat(source) -> llm-chat` 经 `internal/chat` 的 OpenAI-compatible 非流式 Protocol Adapter 发起真实 Chat Completions 调用，完整响应后追加 assistant 消息并持久化 filesystem-backed Conversation Artifact，usage / finish reason / Provider request ID 进入 Node Run diagnostics；认证、限流、网络与协议错误是 Structural Error，失败 Run/Interrupted Run 与已成功进度完整留存。相同规范化语义哈希重复运行复用 Revision，每次创建新 Run，UI 可按 Workflow → Revision → Run → Node Run/Artifact 分层浏览并在重启后查询。删除 Provider/Model 前展示受影响 Workflow、Node 与将移除的每个 Model Slot，删除后悬空 Model UUID 在 Preview 产生 `nodes[i].llm.modelUuid` 字段级 Diagnostic 并阻止 StartRun，直到用户在 Node 表单重新选择；历史 Run Snapshot 仍显示当时 Provider 名称与 Provider Model ID。macOS Provider API Key 已通过注入的 Keychain Secret Adapter 保存，SQLite 只持久化引用，普通 ViewModel 不返回明文或引用；Browser Mock 与测试使用 Memory Adapter 与本地 fixture server。独立 product schema 不读取或复用 workflow/v1 YAML identity。每个已发布 product schema version（5–11）都有代表性旧库 fixture 与顺序升级合同测试：升级保留 Workflow、Draft lock version、Revision hash、Model Slot UUID、删除状态、显式/有效 default 与历史 Run Snapshot，失败升级保持旧库可恢复（user_version 不前移、已执行 DDL 回滚、行原样可读），workflow/v1 定义与 Run history 在升级后仍通过原有查询。P15 日志脱敏与 Crash diagnostics 已完成：`internal/redaction` 共享 seam 在所有日志/错误路径移除已解析 Secret 与敏感 Header，每个真实 Run 写 `runs/<run-id>/logs/run.log`（identity、phase、latency、Provider request ID、脱敏错误），`GenerateDiagnosticsBundle` 显式生成 `runs/<run-id>/diagnostics/` 诊断包（schema/app 版本、Run/Node Run 摘要、脱敏日志副本），永不包含 Prompt、Conversation body 或 Artifact 本体；已知 Secret canary 测试扫描数据库、日志、错误、Artifact 与 bundle 全部无泄漏。人工输入与 Resume 尚未实现；下一票是 `.scratch/product-workflow/issues/16-macos-build-install-upgrade.md`。
