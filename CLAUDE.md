...
**14 后产品化正在实施**：Local Data Root、In-place Project Workspace、`project.code` Workflow Context Binding，以及真实 `go-static-analysis` / `go-coverage-check` Code Quality Check 已落地。两者使用不可变 POSIX Script Bundle 在 Darwin/Linux 的用户 PATH 上原地运行；Coverage 禁用 Go test cache、运行 full scope Go JSON 测试，并按默认 80%（可配置）的 statement coverage 阈值生成严格 Result。Race 与 Complexity 尚未实现。
