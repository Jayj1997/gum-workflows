...
func loadAndValidate(path string) (workflow.Definition, []byte, *node.ExecutorRegistry, *definition.Registry, []validation.Warning, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return workflow.Definition{}, nil, nil, nil, nil, fmt.Errorf("read workflow file: %w", err)
	}

	if err := validation.ValidateSchema(path, data); err != nil {
		return workflow.Definition{}, nil, nil, nil, nil, err
	}
	...
	if err != nil {
		return workflow.Definition{}, nil, nil, nil, nil, fmt.Errorf("load seed definitions: %w", err)
	}
	executors := node.NewExecutorRegistry()
	if err := builtins.RegisterAll(executors); err != nil {
		return workflow.Definition{}, nil, nil, nil, nil, fmt.Errorf("register builtin executors: %w", err)
	}
	if err := builtins.CheckConsistency(executors, defsRegistry); err != nil {
		return workflow.Definition{}, nil, nil, nil, nil, fmt.Errorf("startup consistency check failed:\n%w", err)
	}
	...
	if c, err := llm.LoadDefault(); err != nil {
		if !errors.Is(err, llm.ErrConfigNotFound) {
			return workflow.Definition{}, nil, nil, nil, nil, fmt.Errorf("load llm.yaml: %w", err)
		}
	} else {
		llmConfig = &c
	}
	...
	if err != nil {
		return workflow.Definition{}, nil, nil, nil, nil, fmt.Errorf("semantic validation failed:\n%w", err)
	}
	return def, data, executors, defsRegistry, warnings, nil
}
