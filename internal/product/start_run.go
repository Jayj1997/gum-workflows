...
// NodeRunView is one user-visible fake executor result.
...
// RunView is the completed P9 Product Tracer result.
type RunView struct {
	ID         string          `json:"id"`
...
// StartRun validates and materializes the visible Draft, executes the P9 fake
// conversation tracer, and atomically publishes its persistent history.
func (a *Application) StartRun(ctx context.Context, input StartRunInput) (RunView, error) {
	...
	nodeRuns, runArtifacts, artifactViews, err := a.executeFakeConversation(runID, materialized, now)
	if err != nil {
	...
	for _, nodeRun := range nodeRuns {
		views = append(views, NodeRunView{ID: nodeRun.ID, NodeID: nodeRun.NodeID, NodeDefinition: nodeRun.NodeDefinition, NodeExecutor: nodeRun.NodeExecutor, Status: nodeRun.Status})
	}
...
func (a *Application) materializeLLMSelections(ctx context.Context, content map[string]any) ([]productworkflow.ResolvedLLMSelection, error) {
	...
	for index, value := range nodes {
		...
		selections = append(selections, productworkflow.ResolvedLLMSelection{
			...
			Protocol: resolved.Provider.Protocol, BaseURL: resolved.Provider.BaseURL, ModelUUID: resolved.Model.ID,
...
func (a *Application) executeFakeConversation(runID string, content map[string]any, now time.Time) ([]productworkflow.NodeRun, []productworkflow.RunArtifact, []ArtifactView, error) {
	nodes, _ := content["nodes"].([]any)
	...
	for _, value := range nodes {
		...
		if definition.Kind == nodecatalog.NodeAgent {
			if agent == nil {
				...
				return nil, nil, nil, fmt.Errorf("fake executor supports exactly one llm-chat Node")
			}
		}
	}
	if agent == nil {
		return nil, nil, nil, fmt.Errorf("fake executor requires one llm-chat Node")
	}
	...
	if !valid || sourcePort != "conversation" || stringValue(human, "definition") != "human-chat" {
		return nil, nil, nil, fmt.Errorf("fake executor requires llm-chat.inputs.conversation from human-chat.conversation")
	}
	store, err := artifact.NewFilesystemStore(a.runPaths.ArtifactsDir(runID))
	if err != nil {
		return nil, nil, nil, err
	}
	sourceConversation := productworkflow.Conversation{Messages: []productworkflow.ChatMessage{{Role: "user", Text: "Hello from the P9 fake executor."}}}
	finalConversation := productworkflow.Conversation{Messages: append(append([]productworkflow.ChatMessage{}, sourceConversation.Messages...), productworkflow.ChatMessage{Role: "assistant", Text: "Fake model response."})}
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("write source Conversation Artifact: %w", err)
	}
	...
	nodeRuns := []productworkflow.NodeRun{
		...
		{ID: agentRunID, RunID: runID, NodeID: agentID, NodeDefinition: stringValue(agent, "definition"), NodeExecutor: stringValue(agent, "executor"), Status: "succeeded", Inputs: map[string]artifact.ArtifactRef{"conversation": sourceRef}, Outputs: map[string]artifact.ArtifactRef{"conversation": finalRef}, StartedAt: now, FinishedAt: now},
	}
	items := []productworkflow.RunArtifact{
		...
		{ID: finalArtifactID, RunID: runID, NodeRunID: agentRunID, NodeID: agentID, Port: "conversation", Type: "Conversation", Version: "2", URI: finalRef.URI, CreatedAt: now},
	}
	views := []ArtifactView{
...
func artifactView(item productworkflow.RunArtifact, conversation productworkflow.Conversation) ArtifactView {
