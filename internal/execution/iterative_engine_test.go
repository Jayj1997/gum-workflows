...
func TestBackendCodeVersionsTriggerBoundCheckRounds(t *testing.T) {
	put := func(ctx node.ExecutionContext, name string) (artifact.ArtifactRef, error) {
		return ctx.Store.Put(artifact.Artifact{ID: name, Kind: "KindA"})
	}
	factories := []node.ExecutorFactory{
		fnFactory{definition: "source", outputs: map[string]definition.OutputPort{"seed": {Type: "KindA"}}, create: func(node.Config) (node.Node, error) {
			return callbackNode(func(ctx node.ExecutionContext, _ map[string]artifact.ArtifactRef) (map[string]artifact.ArtifactRef, error) {
				ref, err := put(ctx, "seed")
				return map[string]artifact.ArtifactRef{"seed": ref}, err
			}), nil
		}},
		fnFactory{definition: "backend", inputs: map[string]definition.InputPort{
			"seed": {Type: "KindA"}, "feedback": {Type: "KindA", Optional: true},
		}, outputs: map[string]definition.OutputPort{"code": {Type: "SourceCode"}}, create: func(node.Config) (node.Node, error) {
			return callbackNode(func(node.ExecutionContext, map[string]artifact.ArtifactRef) (map[string]artifact.ArtifactRef, error) {
				return map[string]artifact.ArtifactRef{"code": {
					ID: "code", Kind: artifact.KindSourceCode, URI: "workspace://project",
				}}, nil
			}), nil
		}},
		fnFactory{definition: "check", inputs: map[string]definition.InputPort{"code": {Type: "SourceCode"}}, outputs: map[string]definition.OutputPort{"result": {Type: "KindA"}}, create: func(node.Config) (node.Node, error) {
			return callbackNode(func(ctx node.ExecutionContext, _ map[string]artifact.ArtifactRef) (map[string]artifact.ArtifactRef, error) {
				ref, err := put(ctx, "result")
				return map[string]artifact.ArtifactRef{"result": ref}, err
			}), nil
		}},
		fnFactory{definition: "feedback", inputs: map[string]definition.InputPort{"result": {Type: "KindA"}}, outputs: map[string]definition.OutputPort{"feedback": {Type: "KindA"}}, create: func(node.Config) (node.Node, error) {
			return callbackNode(func(ctx node.ExecutionContext, _ map[string]artifact.ArtifactRef) (map[string]artifact.ArtifactRef, error) {
				ref, err := put(ctx, "feedback")
				return map[string]artifact.ArtifactRef{"feedback": ref}, err
			}), nil
		}},
	}
	...
	def := workflow.Definition{
		APIVersion: workflow.APIVersionV1, Kind: workflow.KindWorkflow,
		Metadata: workflow.Metadata{Name: "backend-check-loop"},
		Nodes: map[string]workflow.NodeSpec{
			"source": {Node: "source"},
			"backend": {Node: "backend", Inputs: map[string]workflow.InputBinding{
				"seed": {From: "source.seed"}, "feedback": {From: "feedback.feedback"},
			}},
			"check": {Node: "check", Inputs: map[string]workflow.InputBinding{
	...
	exec, err := NewEngine(er, dr, artifact.NewMemStore(), nil, WithConvergenceLimit(3)).Run(context.Background(), def)
	if err == nil || exec.Status != StatusFailed {
		t.Fatalf("Run() = %s/%v, want convergence failure after iterative rounds", exec.Status, err)
	}
	checkRuns := append(append([]NodeRun{}, exec.Node("check").History...), exec.Node("check").Current)
	if len(checkRuns) != 3 {
		t.Fatalf("check rounds = %d, want 3 code-triggered rounds", len(checkRuns))
	}
	for i, run := range checkRuns {
		wantVersion := fmt.Sprint(i + 1)
		input := run.Inputs["code"]
		if input.From != "backend.code" || input.Ref.Version != wantVersion {
			t.Errorf("check round %d code input = %+v, want backend.code version %s", run.Round, input, wantVersion)
		}
	}
}

func TestFailedBackendRoundDoesNotPublishCode(t *testing.T) {
	factories := []node.ExecutorFactory{
		fnFactory{definition: "backend", outputs: map[string]definition.OutputPort{"code": {Type: "SourceCode"}}, create: func(node.Config) (node.Node, error) {
			return callbackNode(func(node.ExecutionContext, map[string]artifact.ArtifactRef) (map[string]artifact.ArtifactRef, error) {
				return map[string]artifact.ArtifactRef{"code": {
					ID: "code", Kind: artifact.KindSourceCode, URI: "workspace://project",
				}}, fmt.Errorf("agent failed")
			}), nil
		}},
		fnFactory{definition: "check", inputs: map[string]definition.InputPort{"code": {Type: "SourceCode"}}, create: func(node.Config) (node.Node, error) {
			return callbackNode(func(node.ExecutionContext, map[string]artifact.ArtifactRef) (map[string]artifact.ArtifactRef, error) {
				t.Fatal("check ran after failed backend round")
				return nil, nil
			}), nil
		}},
	}
	...
	def := workflow.Definition{
		APIVersion: workflow.APIVersionV1, Kind: workflow.KindWorkflow,
		Metadata: workflow.Metadata{Name: "failed-backend"},
		Nodes: map[string]workflow.NodeSpec{
	...
	exec, err := NewEngine(er, dr, artifact.NewMemStore(), nil).Run(context.Background(), def)
	if err == nil {
		t.Fatal("Run() = nil error, want backend failure")
	}
	if outputs := exec.Node("backend").Current.Outputs; len(outputs) != 0 {
		t.Errorf("failed backend outputs = %+v, want no published code", outputs)
	}
	if status := exec.Node("check").Current.Status; status != StatusPending {
