...
**14 后 Product Workflow 01–13 与 Run history hardening 已完成**：Wails macOS 产品壳、Browser Mock 与 Desktop Adapter 共享 WorkflowClient / Product Application seam；用户可以在 UI 向 Local Data Root 的 `product.db` 创建 Product Workflow，编辑唯一 Draft，从 Catalog 创作 `human-chat` / `llm-chat`、Input Binding / Control Dependency，并管理 SQLite `Provider -> Models`。Provider 可选 developer/system instructions dialect，旧设置迁移默认 developer；Draft autosave 使用 expected `lock_version` CAS，Preview 在非法 Draft 下仍返回可识别图与聚合 Diagnostics。`StartRun` preflight 校验 UI token、Draft/Model/Secret 与 authored Human input（悬空 Model UUID 产生 `nodes[i].llm.modelUuid` 字段级 Diagnostic 且不 fallback）；通过后以 BeginRun 物化默认 Gum Model UUID、创建/复用 Revision、固定包含 dialect 但不含 Secret 明文的 Run Snapshot，并先持久化 Running Run。Human Node Run/source Artifact 与 in-flight Agent Node Run 在真实 OpenAI-compatible 调用前持久化，随后 Finalize 为 Succeeded、Failed 或 Interrupted；usage / finish reason / Provider request ID、inputs/outputs、时间与 typed error/diagnostics 均可在当前 Run/History UI 查看。Structural Error 会留下 Failed Run；取消或重启遗留调用会留下 Interrupted Run/UnknownOutcome Node Run，保留已成功进度、不自动重放。应用首次 OpenWorkspace 原子协调遗留 Running 状态，旧 Provider 结果不能覆盖 Interrupted；当前版本明确不可 Resume。删除 Provider/Model 前展示受影响 Workflow、Node 与将移除的每个 Model Slot，删除后 Draft 保持原 UUID 并由用户在 Node 表单重新选择，历史 Run Snapshot 仍显示当时 Provider 名称与 Provider Model ID。API Key 经 Keychain Secret Adapter 保存，SQLite 只持久化引用；协议错误在持有实际 Key 的 Adapter 边界精确脱敏。Interaction Error 与同 Run Retry、WaitingHuman、Conversation feedback 和 Resume 尚未实现；下一票是 `.scratch/product-workflow/issues/14-product-schema-upgrade-fixtures.md`。
