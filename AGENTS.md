...
**14 后 Product Workflow 01–07 已完成**：Wails macOS 产品壳、Browser Mock 与 Desktop Adapter 共享 WorkflowClient / Product Application seam；用户可以在 UI 向 Local Data Root 的 `product.db` 创建 Product Workflow，编辑唯一 Draft，从 Catalog 添加和配置 `human-chat` / `llm-chat`，创作 Input Binding / Control Dependency，并管理 SQLite `Provider -> Models`。Draft autosave 使用 expected `lock_version` CAS；Preview 在非法 Draft 下仍返回可识别图与聚合 Diagnostics。P9 `StartRun` 会先校验 UI token、物化默认 Gum Model UUID、创建或复用 immutable Revision、固定无 Secret 的 Run Snapshot，并由 deterministic fake executor 持久化 Run、Node Run 与 filesystem-backed Conversation Artifact；Desktop UI 可直接查看本次 fake 结果。它不读取或复用 workflow/v1 YAML identity，也不包含真实 LLM、人工输入或分层历史浏览。下一票是 `.scratch/product-workflow/issues/08-revision-reuse-run-history-ui.md`。
